...
def detector(net, layer_names, height, width, img, colors, labels,
             boxes=None, confidence=0.5, threshold=0.5, classids=None, idxs=None, infer=True, show_time=False, is_draw=False):
    if infer:
        # Contructing a blob from the input image
        blob = cv.dnn.blobFromImage(img, 1 / 255.0, (416, 416),
                                    swapRB=True, crop=False)
    ...
    # Draw labels and boxes on the image
    if is_draw:
        img_result = draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels)

    return img_result, boxes, confidences, classids, idxs
